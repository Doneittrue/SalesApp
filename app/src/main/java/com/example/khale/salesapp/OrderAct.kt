...
class OrderAct : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var url = "https://30ffdb19.ngrok.io/SalesWeb/get_temp.php?mobile=" + UserInfo.mobile
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        ...
        if (item!!.itemId == R.id.item_cancel) {
            var url = "https://30ffdb19.ngrok.io/SalesWeb/cancel_order.php?mobile" + UserInfo.mobile
            var rq: RequestQueue = Volley.newRequestQueue(this)
            var sr = StringRequest(Request.Method.GET, url, Response.Listener { response ->
                var i: Intent = Intent(this, HomeAct::class.java)
                startActivity(i)
            }, Response.ErrorListener { error ->
                Toast.makeText(this, error.message, Toast.LENGTH_LONG).show()
            })
        ...
        if (item?.itemId == R.id.item_confirm) {
            var url = "https://30ffdb19.ngrok.io/SalesWeb/confirm_order.php?mobile=" + UserInfo.mobile
            var rq: RequestQueue = Volley.newRequestQueue(this)
            var sr = StringRequest(Request.Method.GET, url, Response.Listener { response ->
